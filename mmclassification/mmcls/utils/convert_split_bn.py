...
class SplitBatchNorm2d(torch.nn.ModuleList):
    ...
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        self.main_bn = nn.BatchNorm2d(num_features, eps, momentum, affine, track_running_stats)
        self.aux_bn = nn.BatchNorm2d(num_features, eps, momentum, affine, track_running_stats)

    def forward(self, input: torch.Tensor, domain):
        if domain == 0:
            return self.main_bn(input)
        elif domain == 1:
            return self.aux_bn(input)
...
def convert_splitnorm_model(module, num_splits=2):
    ...
    if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
        ...
        for m in mod:
            m.running_mean = module.running_mean.clone()
            m.running_var = module.running_var.clone()
            m.num_batches_tracked = module.num_batches_tracked.clone()
            if module.affine:
                m.weight.data = module.weight.data.clone().detach()
                m.bias.data = module.bias.data.clone().detach()
