...
class DAEpochBasedRunner(DABaseRunner):
    ...
    def train(self, data_loader_s, data_loader_t, **kwargs):
        ...
        self._max_iters = self._max_epochs * max(len(self.data_loader_s), len(self.data_loader_t))
        ...
        for i in range(self._max_iter_per_epoch):
            ...
            if self.batch_processor is None:
                ...
                try:
                    ...
                    self.iter_t = iter(self.data_loader_t)
                    input_data_t = self.iter_t.__next__()
                self.run_iter(input_data_s, input_data_t, train_mode=True)
    ...
    def run(self, data_loaders_s, data_loaders_t, workflow, max_epochs=None, **kwargs):
        ...
        for i, flow in enumerate(workflow):
            mode, epochs = flow
            if mode == 'train':
                self._max_iters = self._max_epochs * max(len(data_loaders_s[i]), len(data_loaders_t[i]))
                break
